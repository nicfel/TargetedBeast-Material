...
for (logFile in logFiles) {
  ...
  for (header in c("posterior", "likelihood", "prior")) {
    ...
    ess <- ess_tracer_style(subset_log[,header])
    ess.default <- ess_tracer_style(subset_log_default[,header])
    
    if (ess<1 || ess.default<1){
      print("next")
      next
    }
    ...
    mean_val <- quantile(log[round(nrow(log)/2):nrow(log),header], 0.95)
    # get the number of the first sample larger than the mean
    first_sample <- which(log[,header] > mean_val)[1]
    first_sample_default <- which(log.default[,header] > mean_val)[1]
...
for (dataset in unique(posterior_combined$dataset)){
  for (header in c("posterior", "likelihood", "prior")){
    ...
    for (c in seq(1,length(comp_to))){
      ...
      # if (c==3){
        improvement = rbind(improvement, data.frame(x=x_val +(off_from[c] +off_to[c])/2, y=v_off[c],# y=mean(c(ess_to, ess_from))*v_off[c], 
                                                    dataset = dataset, header = header, 
                                                    text_addition = text_addition[c],
...
data_convergence$name <- factor(data_convergence$name, levels = c("combined", "individual"))
...
# plot the ESS values
p = ggplot() + 
  geom_point(data=data_convergence[data_convergence$plot & data_convergence$name=="combined",], aes(x=x, y=ess/time*60*60, fill=method, size=name), color="black", shape = 21) +
  geom_point(data=data_convergence[data_convergence$plot & data_convergence$name=="individual",], aes(x=x, y=ess/time*60*60, fill=method, size=name), color="black", shape = 21) +
  ...
  scale_size_manual(name="computed\nfrom runs", values=c(3, 1)) +
  geom_label(data = improvement,
             aes(x = x, y = y*25, label = paste0(round(improvement, 1))),
             size = 3,
             # make bold
             fontface = "bold",
             color = "black",
             ...
             vjust=0)
plot(p)
ggsave("convergence_longerruns.pdf", p, width = 12, height = 7)
das
...
p = ggplot(data_convergence[data_convergence$plot,], aes(x=x, y=burnin, color = method, fill=method)) + 
  ...
  theme_minimal() +
  scale_color_OkabeIto() +
  geom_segment(data=lines[lines$plot,], aes(x=xfrom, y=from_burnin, xend=xto, yend=to_burnin), color="black", size=0.1) +
  scale_x_continuous(breaks = seq(1, length(datasets)), labels=datasets)
  # geom_text(data=improvement, aes(x=x, y=y.burnin, label=round(burnin, 2)), size=3, color="black", vjust=-2)
plot(p)
ggsave("burnin_longerruns.pdf", p, width = 8, height = 4)
